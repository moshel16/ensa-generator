...
def build_docx(summary: str, topic: str) -> bytes:
    ...
    def run_font(run, size_pt, color, bold=False, font='Arial', italic=False, no_rtl=False):
        ...
        if no_rtl:
            # LTR מוחלט — הסר rtl אם קיים + הגדר lang אנגלית
            for rtl_el in rPr.findall(qn('w:rtl')):
                rPr.remove(rtl_el)
            # lang מפורש = אנגלית, מונע ביאס RTL
    ...
    while i < len(lines):
        ...
        if line.startswith('- ') or line.startswith('* '):
            ...
            def fill_bullets(cell, bs=bullets):
                for b in bs:
                    # הסר backticks מהתחלה לבדיקת is_ltr
                    b_clean = re.sub(r'`([^`]+)`', r'\1', b)
                    # חפש אם יש פקודה + הסבר בעברית (מופרדים ב-— או -)
                    cmd_match = re.match(r'^(`?[a-zA-Z][a-zA-Z0-9\s\-_/.|]+`?)\s*[—–-]+\s*(.+)$', b_clean)
                    if cmd_match and is_ltr_line(cmd_match.group(1).strip()):
                        # שורה 1: פקודה LTR
                        p_cmd = cell_add_para(cell, ltr=True)
                        set_spacing(p_cmd, before=20, after=0)
                        r_cmd = p_cmd.add_run(cmd_match.group(1).strip())
                        run_font(r_cmd, 10, DARK_TEXT, font='Courier New', no_rtl=True)
                        # שורה 2: הסבר RTL
                        p_exp = cell_add_para(cell)
                        set_spacing(p_exp, before=0, after=25)
                        rd = p_exp.add_run('  ')
                        run_font(rd, 10, DARK_TEXT)
                        parse_inline(p_exp, cmd_match.group(2).strip(), default_color=DARK_TEXT)
                    else:
                        para = cell_add_para(cell)
        ...
        if line.startswith('|'):
            ...
            TW = 9200
            if col_count == 2:   cws = [5000, 4200]
            elif col_count == 3: cws = [2800, 3200, 3200]
            elif col_count == 4: cws = [2400, 2200, 2200, 2400]
            elif col_count == 5: cws = [1900, 1700, 1700, 1950, 1950]
            ...
            for ri, row_line in enumerate(rows_data):
                ...
                for ci, cell in enumerate(row.cells[:col_count]):
                    ...
                    cell_ltr = is_ltr_line(txt) and not is_header
                    # בtוך bidiVisual הסדר הפוך: ci=0 = ימין ויזואלי
                    # פקודה LTR → right (=שמאל ויזואלי) + bidi=0
                    # טקסט עברי → left (=ימין ויזואלי) — ברירת מחדל
                    if is_header:
                        jcEl.set(qn('w:val'), 'center')
                        bidi_p = OxmlElement('w:bidi')
                        pPr.insert(0, bidi_p)
                    elif cell_ltr:
                        # פקודה LTR: bidi=0, jc=left = שמאל ויזואלי ב-bidiVisual
                        bidi0 = OxmlElement('w:bidi')
                        bidi0.set(qn('w:val'), '0')
                        pPr.insert(0, bidi0)
                        jcEl.set(qn('w:val'), 'left')
                    else:
                        # עברית RTL: bidi=on, jc=left = ימין ויזואלי
                        bidi_p = OxmlElement('w:bidi')
                        pPr.insert(0, bidi_p)
                        jcEl.set(qn('w:val'), 'left')
                    pPr.append(jcEl)
                    ...
                    run_font(r, 10 if not is_header else 11,
                             ...
                             bold=is_header, no_rtl=cell_ltr)
                    # run_font עם no_rtl=True כבר מסיר rtl
        ...
        if text:
            def fill_text(cell, t=text):
                ...
                if ltr:
                    r = para.add_run(t)
                    run_font(r, 11, DARK_TEXT, font='Courier New', no_rtl=True)
                else:
                    parse_inline(para, t)
