...
def build_docx(summary: str, topic: str) -> bytes:
    ...
    def set_para_bidi(para, jc='left'):
        ...
        jc=left + bidi על הפסקה = RTL נכון
        """
    ...
    def parse_inline(para, text, default_color=None):
        ...
        הכל בעברית — מונחים אנגליים בסוגריים מטופלים כטקסט רגיל.
        """
        ...
        if re.match(r'^\d+\.\s', line):
            ...
            def fill_numbered(cell, its=items):
                for item in its:
                    ...
                    if dm:
                        r1 = para.add_run(dm.group(1))
                        run_font(r1, 11, BLUE, bold=True)
                        r2 = para.add_run(' — ')
                        run_font(r2, 11, DARK_TEXT)
                        parse_inline(para, dm.group(2))
                    else:
